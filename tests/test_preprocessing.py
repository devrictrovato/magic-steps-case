...
class TestDropInvalidColumns:
    ...
    def test_sem_colunas_proibidas_nao_falha(self):
        ...
        # Usa colunas que NÃO estão em columns_to_drop para garantir que
        # drop_invalid_columns não levanta erro e preserva o que não é proibido.
        df = pd.DataFrame({"fase": [2], "score_inde": [7.5]})
        result = pp.transformer.drop_invalid_columns(df)

        assert list(result.columns) == ["fase", "score_inde"]
...
class TestMapTarget:
    """Testa map_target() e bucket_defasagem() para o target numérico ternário."""

    def test_mapeamento_numerico_preservado(self):
        """map_target com target_map={} deve preservar os valores numéricos
        convertendo-os para dtype numérico sem alterar os valores."""
        df = pd.DataFrame({
            data_config.target_column: [-2, -1, 0, 1],
        })
        result = pp.transformer.map_target(df)

        assert result[data_config.target_column].tolist() == [-2, -1, 0, 1]

    def test_valor_nao_numerico_gera_nan(self):
        """Valor não-numérico no target deve resultar em NaN após coerção."""
        df = pd.DataFrame({
            data_config.target_column: [-1, "invalido", 0],
        })
        result = pp.transformer.map_target(df)
    ...
    def test_bucket_defasagem_classes_ternarias(self):
        """bucket_defasagem() converte valores brutos em classes 0/1/2."""
        df = pd.DataFrame({
            data_config.target_column: [-5, -1, 0, 1, 2],
        })
        result = pp.transformer.bucket_defasagem(df)

        assert result[data_config.target_column].tolist() == [0, 0, 1, 2, 2]

    def test_bucket_defasagem_apenas_negativos(self):
        """Todos negativos → classe 0 (atraso)."""
        df = pd.DataFrame({data_config.target_column: [-3, -2, -1]})
        result = pp.transformer.bucket_defasagem(df)

        assert all(result[data_config.target_column] == 0)

    def test_bucket_defasagem_apenas_positivos(self):
        """Todos positivos → classe 2 (avanço)."""
        df = pd.DataFrame({data_config.target_column: [1, 2]})
        result = pp.transformer.bucket_defasagem(df)

        assert all(result[data_config.target_column] == 2)

    def test_nao_modifica_original(self):
        """bucket_defasagem não deve mutar o DataFrame original."""
        df = pd.DataFrame({data_config.target_column: [-1, 0, 1]})
        original = df[data_config.target_column].tolist()
        pp.transformer.bucket_defasagem(df)

        assert df[data_config.target_column].tolist() == original
...
class TestBuildPreprocessor:
    ...
    def test_fit_transform_shape(self, preprocessor_and_data):
        """Saída deve ter n_rows × 16 (13 num + 3 cat).
        Features numéricas: fase, ano_ingresso, score_inde, score_iaa,
        score_ieg, score_ips, score_ida, score_ipv, score_ian, nota_cg,
        nota_cf, nota_ct, num_avaliacoes (13).
        idade e defasagem foram removidas (dropped / virou target).
        Categóricas: turma, genero, pedra_modal (3). Total = 16."""
        pre, X, _ = preprocessor_and_data
        n_num = len([c for c in data_config.final_feature_order
                     if c not in data_config.categorical_columns])
        n_cat = len(data_config.categorical_columns)
        X_t = pre.fit_transform(X)

        assert X_t.shape == (len(X), n_num + n_cat)

    def test_fit_transform_range_numericas(self, preprocessor_and_data):
        """Após fit_transform, as colunas numéricas devem estar em [0, 1]."""
        pre, X, _ = preprocessor_and_data
        X_t = pre.fit_transform(X)
        n_num = len([c for c in data_config.final_feature_order
                     if c not in data_config.categorical_columns])
...
class TestPreprocessorJoblib:
    ...
    O joblib atual foi gerado com o pipeline de defasagem ternária:
      - Entrada: 16 colunas (final_feature_order)
      - Saída: 16 features (13 numéricas + 3 categóricas)
      - idade e defasagem (raw) removidas do pipeline
    """
    ...
    def test_n_features_in(self, preprocessor):
        """Deve esperar 16 colunas de entrada (final_feature_order).
        idade e defasagem foram removidas do pipeline de features."""
        expected = len(data_config.final_feature_order)
        assert preprocessor.n_features_in_ == expected

    def test_output_dim(self, preprocessor):
        """Saída deve ter 16 features (13 num + 3 cat)."""
        names = preprocessor.get_feature_names_out()
        n_num = len([c for c in data_config.final_feature_order
                     if c not in data_config.categorical_columns])
        n_cat = len(data_config.categorical_columns)
        assert len(names) == n_num + n_cat

    def test_remainder_drop(self, preprocessor):
        assert preprocessor.remainder == "drop"

    def test_idade_nao_esta_nas_features(self, preprocessor):
        """idade não deve aparecer nas colunas de entrada do preprocessor."""
        num_cols = preprocessor.transformers_[0][2]
        cat_cols = preprocessor.transformers_[1][2]
        all_cols = list(num_cols) + list(cat_cols)
        assert "idade" not in all_cols

    def test_defasagem_nao_esta_nas_features(self, preprocessor):
        """defasagem é o target — não deve aparecer como feature."""
        num_cols = preprocessor.transformers_[0][2]
        assert "defasagem" not in num_cols
    ...
    def test_min_fase_e_zero(self, preprocessor):
        """fase mínima na base de treino é 0."""
        scaler = preprocessor.transformers_[0][1]
        num_cols = preprocessor.transformers_[0][2]
        idx = num_cols.index("fase")
    ...
    def _make_input(self, **overrides) -> pd.DataFrame:
        """Monta um DataFrame de 1 linha com os 16 campos de final_feature_order.
        idade, ano_nascimento e defasagem foram removidos do pipeline de features:
          - idade:         redundante com ano_ingresso
          - ano_nascimento: dropado antes do preprocessor
          - defasagem:     é agora o target (não entra no ColumnTransformer)
        """
        base = {
            "fase": 2, "ano_ingresso": 2021,
            "score_inde": 7.5, "score_iaa": 8.5, "score_ieg": 8.0,
            "score_ips": 7.0, "score_ida": 6.5, "score_ipv": 7.8,
            "score_ian": 5.0, "nota_cg": 400, "nota_cf": 70,
            "nota_ct": 6, "num_avaliacoes": 3,
    ...
    def test_transform_shape(self, preprocessor):
        """Output deve ser (1, n_features) onde n_features = 13 num + 3 cat = 16."""
        n_num = len([c for c in data_config.final_feature_order
                     if c not in data_config.categorical_columns])
        n_cat = len(data_config.categorical_columns)
        out = preprocessor.transform(self._make_input())

        assert out.shape == (1, n_num + n_cat)
    ...
    def test_transform_valores_max_produzem_um(self, preprocessor):
        ...
        inp = self._make_input(
            ...
            nota_ct=18, num_avaliacoes=4,
        )
        out = preprocessor.transform(inp).flatten()
        n_num = len([c for c in data_config.final_feature_order
                     if c not in data_config.categorical_columns])

        np.testing.assert_allclose(out[:n_num], 1.0, atol=1e-7)
    ...
    def test_transform_ordinal_menina_é_zero(self, preprocessor):
        ...
        # posição 14 = cat__genero (após 13 numéricas + turma na pos 13)
        cat_cols = preprocessor.transformers_[1][2]
        pos = len(preprocessor.transformers_[0][2]) + cat_cols.index("genero")
        assert out[pos] == 0.0
    ...
    def test_transform_ordinal_turma_b_é_um(self, preprocessor):
        ...
        cat_cols = preprocessor.transformers_[1][2]
        pos = len(preprocessor.transformers_[0][2]) + cat_cols.index("turma")
        assert out[pos] == 1.0
    ...
    def test_transform_ordinal_pedra_topazio_é_dois(self, preprocessor):
        ...
        cat_cols = preprocessor.transformers_[1][2]
        pos = len(preprocessor.transformers_[0][2]) + cat_cols.index("pedra_modal")
        assert out[pos] == 2.0
    ...
    def test_transform_ano_ingresso_normalizado(self, preprocessor):
        """ano_ingresso=2019 deve mapear para (2019-min)/(max-min)."""
        inp = self._make_input(ano_ingresso=2019)
        out = preprocessor.transform(inp).flatten()
        scaler   = preprocessor.transformers_[0][1]
        num_cols = preprocessor.transformers_[0][2]
        idx      = num_cols.index("ano_ingresso")

        min_val = scaler.data_min_[idx]
        max_val = scaler.data_max_[idx]
        expected = (2019 - min_val) / (max_val - min_val)
        np.testing.assert_allclose(out[idx], expected, atol=1e-6)

    def test_transform_batch_shape(self, preprocessor):
        """Batch de 5 linhas deve produzir (5, n_features)."""
        n_num = len(preprocessor.transformers_[0][2])
        n_cat = len(preprocessor.transformers_[1][2])
        rows = pd.concat([self._make_input(nota_cg=100 * i) for i in range(1, 6)])
        out = preprocessor.transform(rows)

        assert out.shape == (5, n_num + n_cat)
